...
OK, you got your unread emails retrieved from the inbox. But where are they exactly? Yes, in a variable you specified. What you should know is that this variable is not ordinary, because it’s designed to store many items (messages) and each of them has typical email components such as from/to email addresses, message subject, message body or date received. We say that this is a [list](https://github.com/G1ANT-Robot/G1ANT.Manual/blob/develop/G1ANT.Language/Structures/ListStructure.md) variable (contains more than one element) and each of its elements is of [mail](https://github.com/G1ANT-Robot/G1ANT.Manual/blob/develop/G1ANT.Language/Structures/MailStructure.md) structure.
...
Remember the asterisk called Search Place? You learned it in the [Basics](https://github.com/G1ANT-Robot/G1ANT.Manual/blob/develop/g1ant-language/02-language-basics.md): it serves as a wildcard for all characters. You can enter this character from `Insert/Search Place` menu, with **Ctrl+8** keyboard shortcut or by clicking the `✱` icon on the toolbar.
