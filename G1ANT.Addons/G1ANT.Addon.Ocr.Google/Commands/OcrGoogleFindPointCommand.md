...
This command finds a specified text on the active screen and returns its position in a [point](../../G1ANT.Language/G1ANT.Language/Structures/PointStructure.md) format. The point will be set in the middle of found text, i.e. if the text was found in rectangular area 0⫽0⫽20⫽20, the result will be 10⫽10.
...
> **Note:** In order to use any `ocrgoogle.` command, you have to be logged in to the Google Cloud Services with the [`ocrgoogle.login`](../G1ANT.Addon.Ocr.Google/Commands/OcrGoogleLoginCommand.md) command.
